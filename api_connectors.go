...
type ApiSearchConnectorJobsRequest struct {
	ctx context.Context
	ApiService *ConnectorsAPIService
	connectorId string
	limit *int32
...
// Maximum number of objects to return per query. The value must be between 1 and 1000. Default is 100.
func (r ApiSearchConnectorJobsRequest) Limit(limit int32) ApiSearchConnectorJobsRequest {
	r.limit = &limit
	return r
}

// Cursor to fetch the next or previous page of results. The value of this property must be extracted from the &#39;prev_cursor&#39; or &#39;next_cursor&#39; property of a PaginatedResponseMetadata which is contained in the response of list and search API endpoints.
func (r ApiSearchConnectorJobsRequest) Cursor(cursor string) ApiSearchConnectorJobsRequest {
	r.cursor = &cursor
	return r
}

// The field to sort results by. A property name with a prepended &#39;-&#39; signifies a descending order.
func (r ApiSearchConnectorJobsRequest) Sort(sort string) ApiSearchConnectorJobsRequest {
	r.sort = &sort
	return r
}
...
func (r ApiSearchConnectorJobsRequest) SearchBody(searchBody SearchBody) ApiSearchConnectorJobsRequest {
	r.searchBody = &searchBody
	return r
}

func (r ApiSearchConnectorJobsRequest) Execute() (*ListConnectorsMaskingJobsResponse, *http.Response, error) {
	return r.ApiService.SearchConnectorJobsExecute(r)
}

/*
SearchConnectorJobs Search jobs that use this connector

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param connectorId The ID of the Connector.
 @return ApiSearchConnectorJobsRequest
*/
func (a *ConnectorsAPIService) SearchConnectorJobs(ctx context.Context, connectorId string) ApiSearchConnectorJobsRequest {
	return ApiSearchConnectorJobsRequest{
		ApiService: a,
		ctx: ctx,
		connectorId: connectorId,
...
// Execute executes the request
//  @return ListConnectorsMaskingJobsResponse
func (a *ConnectorsAPIService) SearchConnectorJobsExecute(r ApiSearchConnectorJobsRequest) (*ListConnectorsMaskingJobsResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *ListConnectorsMaskingJobsResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ConnectorsAPIService.SearchConnectorJobs")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/connectors/{connectorId}/masking-jobs/search"
	localVarPath = strings.Replace(localVarPath, "{"+"connectorId"+"}", url.PathEscape(parameterValueToString(r.connectorId, "connectorId")), -1)
