/*
Delphix DCT API

Delphix DCT API

API version: 3.9.0
Contact: support@delphix.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package delphix_dct_api

import (
	"encoding/json"
)

// checks if the LocationCompatibleEnvironmentsResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &LocationCompatibleEnvironmentsResponse{}

// LocationCompatibleEnvironmentsResponse struct for LocationCompatibleEnvironmentsResponse
type LocationCompatibleEnvironmentsResponse struct {
	Items []Environment `json:"items,omitempty"`
}

// NewLocationCompatibleEnvironmentsResponse instantiates a new LocationCompatibleEnvironmentsResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewLocationCompatibleEnvironmentsResponse() *LocationCompatibleEnvironmentsResponse {
	this := LocationCompatibleEnvironmentsResponse{}
	return &this
}

// NewLocationCompatibleEnvironmentsResponseWithDefaults instantiates a new LocationCompatibleEnvironmentsResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewLocationCompatibleEnvironmentsResponseWithDefaults() *LocationCompatibleEnvironmentsResponse {
	this := LocationCompatibleEnvironmentsResponse{}
	return &this
}

// GetItems returns the Items field value if set, zero value otherwise.
func (o *LocationCompatibleEnvironmentsResponse) GetItems() []Environment {
	if o == nil || IsNil(o.Items) {
		var ret []Environment
		return ret
	}
	return o.Items
}

// GetItemsOk returns a tuple with the Items field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *LocationCompatibleEnvironmentsResponse) GetItemsOk() ([]Environment, bool) {
	if o == nil || IsNil(o.Items) {
		return nil, false
	}
	return o.Items, true
}

// HasItems returns a boolean if a field has been set.
func (o *LocationCompatibleEnvironmentsResponse) HasItems() bool {
	if o != nil && !IsNil(o.Items) {
		return true
	}

	return false
}

// SetItems gets a reference to the given []Environment and assigns it to the Items field.
func (o *LocationCompatibleEnvironmentsResponse) SetItems(v []Environment) {
	o.Items = v
}

func (o LocationCompatibleEnvironmentsResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o LocationCompatibleEnvironmentsResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Items) {
		toSerialize["items"] = o.Items
	}
	return toSerialize, nil
}

type NullableLocationCompatibleEnvironmentsResponse struct {
	value *LocationCompatibleEnvironmentsResponse
	isSet bool
}

func (v NullableLocationCompatibleEnvironmentsResponse) Get() *LocationCompatibleEnvironmentsResponse {
	return v.value
}

func (v *NullableLocationCompatibleEnvironmentsResponse) Set(val *LocationCompatibleEnvironmentsResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableLocationCompatibleEnvironmentsResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableLocationCompatibleEnvironmentsResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableLocationCompatibleEnvironmentsResponse(val *LocationCompatibleEnvironmentsResponse) *NullableLocationCompatibleEnvironmentsResponse {
	return &NullableLocationCompatibleEnvironmentsResponse{value: val, isSet: true}
}

func (v NullableLocationCompatibleEnvironmentsResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableLocationCompatibleEnvironmentsResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


